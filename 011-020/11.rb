...
max = 0
grid.each do |row|
	sub_max = ((0...row.length - 3).map do |idx|
		row[idx] * row[idx + 1] * row[idx + 2] * row[idx + 3]
	end).max
	max = sub_max if max < sub_max
end

grid[0].length.times do |col_idx|
	sub_max = ((0...grid.length - 3).map do |row_idx|
		grid[row_idx][col_idx] * grid[row_idx + 1][col_idx] * grid[row_idx + 2][col_idx] * grid[row_idx + 3][col_idx]
	end).max
	max = sub_max if max < sub_max
end

(grid[0].length - 3).times do |row_idx|
	sub_max = ((0...grid.length - 3).map do |col_idx|
		[
			grid[row_idx][col_idx] * grid[row_idx + 1][col_idx + 1] * grid[row_idx + 2][col_idx + 2] * grid[row_idx + 3][col_idx + 3],
			grid[row_idx + 3][col_idx] * grid[row_idx + 2][col_idx + 1] * grid[row_idx + 1][col_idx + 2] * grid[row_idx][col_idx + 3]
		].max
	end).max
	max = sub_max if max < sub_max
end
